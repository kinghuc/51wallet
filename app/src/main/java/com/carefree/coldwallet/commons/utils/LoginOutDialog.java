...
/*--------------------------------------------------------------------
  文 件 名：LoginOutDialog
  作 　 者：HuangXiJun (黄夕君)
  版本日期：V1.0,  2018/04/18 (版本号+逗号＋日期，注：日期格式：YYYY－MMM－DD，即月用英文表示，尽量减少异意)
  模块功能：退出帐号弹框dialog
---------------------------------------------------------------------*/
public class LoginOutDialog extends Dialog{
    ...
    public LoginOutDialog(Context context) {
        ...
        mContext = context;
        customView = LayoutInflater.from(context).inflate(R.layout.dialog_login_layout, null);
    }
