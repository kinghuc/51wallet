...
/*--------------------------------------------------------------------
  文 件 名：WalletPwdErrorDialog
  作 　 者：HuangXiJun (黄夕君)
  版本日期：V1.0,  2018/04/11 (版本号+逗号＋日期，注：日期格式：YYYY－MMM－DD，即月用英文表示，尽量减少异意)
  模块功能：钱包密码错误弹框
---------------------------------------------------------------------*/
public class WalletPwdErrorDialog extends Dialog{
    private View customView;
    private TextView content;
    private Context mContext;

    public WalletPwdErrorDialog(Context context) {
        super(context,R.style.ErrorDialog);
        mContext = context;
        customView = LayoutInflater.from(context).inflate(R.layout.dialog_wallet_pwd_error, null);
    }
